...
const EOPBCTemplate = artifacts.require('EOPBCTemplate')
...
module.exports = async callback => {
  try {
    if (process.argv[4] === 'rpc') {
      ...
      const template = await EOPBCTemplate.new(daoFactory.address, ens, miniMeFactory.address, aragonId)
      ...
      const template = await EOPBCTemplate.new(daoFactory, ens, miniMeFactory, aragonId, { from: owner })
      console.log(template.address)
    } else {

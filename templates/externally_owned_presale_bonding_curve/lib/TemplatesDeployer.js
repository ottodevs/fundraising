...
module.exports = class TemplateDeployer {
  ...
  async deploy(templateName, contractName) {
    ...
    const { template, ensAddress } = await this.deployTemplate(contractName)
    await this.registerDeploy(templateName, template)
    return { templateAddress: template.address, ensAddress }
  }
  ...
  async deployTemplate(contractName) {
    ...
    return { template: template, ensAddress: this.ens.address }
  }
